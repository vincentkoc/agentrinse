...
the approved filesystem, Git metadata, and offline database changes. `0.8.1`
...
database records remain report-only. `0.8.1` supports explicit offline
...
agentrinse `0.8.1` never removes:
...
`0.8.1` ships the complete audit → plan → revalidate → apply loop, safe

...
async function loadNativeExchange(platform: NodeJS.Platform): Promise<NativeRename> {
  if (!["darwin", "linux"].includes(platform)) {
    const error = new Error(
      `atomic path exchange is unsupported on ${platform}`,
    ) as NodeJS.ErrnoException;
    error.code = "ENOTSUP";
    throw error;
  }
  ...
  const libc = koffi.load(null);
  if (platform === "darwin") {
    const renameSwap = libc.func(
      "int renamex_np(const char *source, const char *destination, unsigned int flags)",
    );
    return (source, destination) => ({
      result: renameSwap(source, destination, 0x0000_0002) as number,
      errno: koffi.errno(),
      errnoNames: koffi.os.errno,
    });
  }

  let renameExchange: ReturnType<typeof libc.func>;
  try {
    renameExchange = libc.func(
      "int renameat2(int olddirfd, const char *source, int newdirfd, const char *destination, unsigned int flags)",
    );
  } catch (error) {
    const unsupported = new Error(
      "the Linux C library does not expose renameat2 for atomic path exchange",
      { cause: error },
    ) as NodeJS.ErrnoException;
    unsupported.code = "ENOTSUP";
    throw unsupported;
  ...
  return (source, destination) => ({
    result: renameExchange(-100, source, -100, destination, 0x0000_0002) as number,
    errno: koffi.errno(),
    errnoNames: koffi.os.errno,
  });
}
...
export async function exchangePaths(
  source: string,
  destination: string,
  platform: NodeJS.Platform = process.platform,
): Promise<void> {
  let nativeExchangePromise = nativeExchangePromises.get(platform);
  if (nativeExchangePromise === undefined) {
    nativeExchangePromise = loadNativeExchange(platform);
    nativeExchangePromises.set(platform, nativeExchangePromise);
  }
  const nativeExchange = await nativeExchangePromise;
  const result = nativeExchange(source, destination);
  if (result.result !== 0) {
    throw renameError(source, destination, result.errnoNames, result.errno);
  }
}
